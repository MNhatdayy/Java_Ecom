...
export const createProduct = async(values, imageUrl, moreImages, navigate, form, setImageUrl, setMoreImages, message,categories) => {
    try {
        ...
        const response = await axios.post('http://localhost:8099/api/products', formData, {
            headers: {
...
export const updateProduct = async(id, values, imageUrl, moreImages, navigate, setLoading, message, categories) =>{
    try {
        setLoading(true);
        const { categoryId, ...restValues } = values;
        const selectedCategory = categories.find((category) => category.id === categoryId);
