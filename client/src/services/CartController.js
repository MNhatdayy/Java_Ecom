...
export const updateCart = async function (cartId, quantity) {
  ...
  export const checkout = async (cartItems, setCartItems) => {
    try {
      ...
      const confirmOrder = window.confirm('Do you want to proceed with checkout?');
      if (confirmOrder) {
  ...
  export const removeFromCart = async (cartItems, setCartItems) => {
    try {
      const shouldRemove = window.confirm('Are you sure you want to remove all items from the cart?');
      if (shouldRemove) {
        setCartItems([]);
